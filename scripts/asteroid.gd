...
func _ready():
	...
	linear_velocity = 1000* Vector3(rng.randf_range(-1,1), rng.randf_range(-1,1), rng.randf_range(-1,1))
...
func _physics_process(delta):
	...
	if global_position.length() > 10.0:
		applied_force = - global_position * (mass * SUN_MASS * GRAVITATIONAL_CONSTANT) / (SUN_TO_METERS * global_position.length()**3)
